...
public partial class MainWindow : Window
{
    ...
    private async void BtnGo_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if (IsBakMode)
            {
                ...
                await RunBakConversionAsync(inputFiles[0], outputFolder, suffix, tables, data, _cts.Token);
            }
            else
            {
        ...
        CancellationToken ct)
    {
        ...
        Log($"  Found {segments.Count} SQL segment(s). Converting…");

        for (int i = 0; i < segments.Count; i++)
        {
            ct.ThrowIfCancellationRequested();
            double pct = 60 + (i + 1) * 40.0 / Math.Max(segments.Count, 1);
            AppLog($"  Converting segment {i + 1}/{segments.Count}…");
            Status($"Converting segment {i + 1} of {segments.Count}  ({pct:F0}%)");

            string converted = SqlConverter.ConvertToMariaDb(segments[i]);
            await writer.WriteLineAsync(converted);
            await writer.WriteLineAsync();
            SetProgress(pct);
        }
        ...
        SetProgress(100);
        AppLog($"BAK conversion complete. Output: {outFile}");
        Status($"✔  Complete — {outFile}");
        Log($"✔  Done. Output: {outFile}");
    }
}
