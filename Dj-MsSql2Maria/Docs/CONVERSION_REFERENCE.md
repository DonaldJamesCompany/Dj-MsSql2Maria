# MS SQL to MariaDB 1.0.0 (Dj-MsSql2Maria) — SQL Conversion Reference
...
| Category | Meaning |
|---|---|
| 🗑️ **REMOVE** | The MS SQL token/statement is found and **deleted entirely** from the output — nothing replaces it |
| ➕ **ADDED** | Something **new is inserted** into the output that was not present in the original MS SQL (e.g. a comment marker, a structural fix) |
...
- [➕ ADDED](#-added)
  - [A1 — Comment Markers](#a1--comment-markers-converted-hints--separators)
  - [A2 — Output File Header](#a2--output-file-header)
  - [A3 — Structural Fix: Trailing Comma](#a3--structural-fix-trailing-comma)
- [🔄 CONVERT](#-convert)
...
- [⚠️ NOT CONVERTED](#️-not-converted--manual-review-required)
...
### A1 — Comment Markers (Converted Hints & Separators)

Some MS SQL constructs cannot be translated but are too important to silently delete.
They are converted to inline comments so they remain visible for manual review.

| MS SQL original | What is added to output | Why a comment instead of removal |
|---|---|---|
| `GO` (batch separator on its own line) | `-- GO` | Marks every original batch boundary for review |
| `WITH (NOLOCK)` | `/* WITH(NOLOCK) */` | Flags dirty-read hints; InnoDB MVCC makes them irrelevant but the reviewer should know |
| `SELECT TOP n …` | `SELECT /* TOP n – move LIMIT n to end of query */ …` | `TOP` cannot be mechanically relocated; comment is a TODO marker |

### A2 — Output File Header

Every generated output file begins with a header block **not** present in the source:

```sql
-- Generated by Dj-MsSql2Maria on YYYY-MM-DD HH:MM:SS
...
| MS SQL | → | MariaDB | Notes |
|---|:---:|---|---|
| `SELECT TOP n …` | → | `SELECT /* TOP n – move LIMIT n to end of query */ …` | ⚠ Manual fix required — add LIMIT n at end of query |
| `SELECT TOP (n) …` | → | Same comment form | Same manual fix required |
...
| `IF OBJECT_ID('tbl') IS NOT NULL DROP TABLE tbl` | → | `DROP TABLE tbl -- converted from IF OBJECT_ID` | Add IF EXISTS manually for safety |
| `IF OBJECT_ID('proc','P') IS NOT NULL DROP PROCEDURE proc` | → | `DROP PROCEDURE proc -- converted from IF OBJECT_ID` | Add IF EXISTS manually for safety |

---

## ⚠️ NOT CONVERTED — Manual Review Required
